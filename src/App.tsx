...
export const App: React.FC = () => {
  ...
  // 3. 단어가 로컬에 0개이고 온라인인 경우, 즉시 Supabase 클라우드에서 자동 복구 동기화
  useEffect(() => {
    if (allWords.length === 0 && isOnline) {
      console.log('[Auto-Recovery] 로컬 단어 목록 0개 감지 -> 클라우드에서 즉시 자동 복구 동기화 실행');
      syncFromCloud(user?.id);
    }
  }, [allWords.length, isOnline, user?.id]);
          ...
          <div>
            {allWords.length === 0 ? (
              <div className="max-w-md mx-auto my-12 p-8 bg-white rounded-3xl border border-slate-200 text-center space-y-4 shadow-sm">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-800">단어 목록을 불러오는 중이거나 비어 있습니다</h3>
                  <p className="text-xs text-slate-500 mt-1">
                    클라우드와 연결하여 최신 단어를 동기화하거나 새 단어를 추가해 보세요.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row items-center justify-center gap-2">
                  <button
                    onClick={() => syncFromCloud(user?.id)}
                    className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-indigo-600 text-white font-bold text-xs hover:bg-indigo-700 shadow-xs active:scale-95 transition-all cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <RotateCcw className="w-3.5 h-3.5" />
                    <span>클라우드 단어 불러오기</span>
                  </button>
                  <button
                    onClick={() => {
                      setActiveTab('manage');
                    ...
                    className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-semibold text-xs hover:bg-slate-200 transition-all cursor-pointer"
                  >
                    단어 직접 추가하기
                  </button>
                </div>
